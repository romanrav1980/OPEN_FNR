...
- help-сноски для значимых UI-элементов: фильтров, действий, статусов, метрик, предупреждений, графиков, таблиц и полей форм;
- каждая help-сноска должна объяснять, что делает элемент, зачем он нужен, какой бизнес-процесс поддерживает и где находится постановка задачи или процессная спецификация;
- help-сноска должна содержать ссылки на связанные документы проекта и process artifacts: BPMN, DMN, CMMN, decision record или sprint specification;
- отсутствие скрытых black-box решений.

### 5.1. Правило Help-Сносок

Для каждого рабочего экрана должен быть определен реестр help-сносок. Реестр хранится рядом с UI-модулем или в общей конфигурации UI-контента и должен быть доступен тестам.

Минимальная структура help-сноски:

| Поле | Описание |
| --- | --- |
| `ui_element_id` | стабильный идентификатор элемента |
| `title` | короткое название подсказки |
| `short_help` | краткое объяснение на рабочем экране |
| `business_purpose` | какую бизнес-задачу решает элемент |
| `process_refs` | ссылки на BPMN/DMN/CMMN или Process Engine artifact |
| `spec_refs` | ссылки на ТЗ, sprint plan, decision record или бизнес-спецификацию |
| `roles` | роли, которым элемент доступен |
| `audit_event` | audit event, если действие меняет данные или статус |

Help-сноски обязательны для:

- кнопок бизнес-действий: approve, reject, publish, export, recalculate, apply, rollback;
- фильтров, влияющих на расчеты и выборку данных;
- статусов и warning/error badges;
- метрик WAPE, Bias, Service Level, Stock Cost Impact, Waste, OOS, overstock;
- графиков прогноза, пополнения, projected stock, capacity, diagnostics;
- полей форм промо, пополнения, корректировок, supplier collaboration, store tasks;
- элементов Process Navigator, показывающих BPMN step, task, SLA, conformance, version и alert.

Help-сноска должна быть доступна с клавиатуры и screen reader. Если элемент скрыт по RBAC, help-сноска не должна раскрывать чувствительные данные или недоступное действие.
